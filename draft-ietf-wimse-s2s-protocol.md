...
Pre-WIMSE, many current use cases don't allow for fully granular authentication. For instance, with mutual TLS,
there's often no clear way to map a Request URI to the SubjectAltName value in the server certificate. This means the client can only verify
if the certificate is valid within a trust domain, not if it's tied to a specific workload.
...
- There needs to be a two-way mapping between the Request URI used to access a workload and its workload identifier.
...
Implementations MUST allow for defining this mapping between the Request URI and the workload identifier (e.g., through
